...
class Path1Protocol:
    ...
    async def send_update(self):
        ...
        while True:
            ...
            # print(f"Message size is {len(msg)}")
...
if __name__ == '__main__':
    ...
    print("Starting the path 2 server")
    # listen = loop.create_datagram_endpoint(
    #        Path1Protocol, local_addr=('55::5', 12347))
    # transport, protocol = loop.run_until_complete(listen)
    sockfile = '/home/ubuntu/path2.sock'
    path2sock = os.open(sockfile, os.O_WRONLY)

    try:
        transport, protocol = loop.run_until_complete(loop.create_datagram_endpoint(
            lambda: Path1Protocol(path2sock),
            local_addr=('55::5', 12347),
            reuse_address=True))
        loop.run_forever()
