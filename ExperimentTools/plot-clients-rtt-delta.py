...
if __name__ == "__main__":
    ...
    df['Receive Time'] = (df['Receive Time'] - df['Receive Time'].min()).dt.total_seconds()
    ...
    ax1.set_ylim(ymin=0)
    ax1.set_xlabel('Time elapsed (%M:%S)')
    ax1.set_ylabel('RTT (ms)')
    ...
    ax1.set_title(f"Scenario: RTT of clients when BGP's default path for all clients is bad\n"
                  f"Overwatch is running and period is 12 seconds" )
