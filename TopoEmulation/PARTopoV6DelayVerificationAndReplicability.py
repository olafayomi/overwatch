...
"""
This IPMininet topology script tests is to be used for verifying and validating
that link being modified has an effect on performance-aware routing performed by
the  PAR controller in AS3. The link between AS4R1 and AS4R2 is initially set with
the lowest delay value and the delay is randomly set to a value between 1ms and 49ms 
every 90 seconds. This is lower than 50ms and 60 secs. A list containing numbers between
1 and 35900 is used to seed a random delay value to ensure that the delay values are 
the same for every run. This should provide similar results.
"""
if __name__ == '__main__':
    ...
    try:
        ...
        #IPCLI(net)
        net["as6h1"].cmd("./tools/D-ITG-2.8.1-r1023/bin/ITGRecv -l ~/DITG-LOGS/AS6H1-RECV-20210417-1.log &> /dev/null &")
        time.sleep(240)
        net["as2r1"].cmd("./tools/D-ITG-2.8.1-r1023/bin/ITGSend -T TCP -a 2001:4521::2 -c 100 -C 10 -t 3600000 -rp 465 -l ~/DITG-LOGS/AS2R1-PAR-FLOW-SEND-20210417-1.log &> /dev/null &")
        net["as1r1"].cmd("./tools/D-ITG-2.8.1-r1023/bin/ITGSend -T TCP -a 2001:4521::2 -c 100 -C 10 -t 3600000 -l ~/DITG-LOGS/AS1R1-NORMAL-FLOW-SEND-20210417-1.log &> /dev/null &")
