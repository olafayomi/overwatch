...
class FloodProtocol(OspfProtocol):
            ...
            for lsa in lsas:
                ...
                if exist_lsa is None or self.judge_new_lsa(exist_lsa['H'], lsas[lsa]['H']) == lsas[lsa]['H']:
                    ...
                    if ls in lslist and age == MAXAGE:
                        ...
                        #
                        # =========================================
                        # XXX: LS RECORD CHANGE OCCURED UPDATE TOPO
                        # =========================================
                        #
                        del lslist[ls]
                    else:
                    ...
                    # If the state of the NSM is full then perform the topo update
                    # oterhwise wait for the state to be set to full
                    if self.nsm.state == NSM_STATE["NSM_Full"]:
                        LOG.info("LSU caused LS record change to occur, recalculate topo")
                        self.nsm.ism.ai.topo_change()
    ...
    def gen_lsack(self, lsas):
        ...
        ospf_packet = OSPF(
            ...
            type=5,             # 5 for lsack
            area=self.area,
            len=len(lsackdata)+len(OSPF()),
            router=self.rid,
            auth=b'',
            data=lsackdata
        )
        return ospf_packet.__bytes__()
